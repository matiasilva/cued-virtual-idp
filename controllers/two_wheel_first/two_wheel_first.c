...
int main(int argc, char **argv) {
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    float targetAngle;
    if(sqDistance <= (circleRadius+TOLERANCE)*(circleRadius+TOLERANCE)){
      if(sqDistance <= (circleRadius-TOLERANCE)*(circleRadius-TOLERANCE)){ // within
      
      } else { // on perimeter
        
      }
    } else { // outside
      double theta = CustomATan(delta.x, delta.z);
      float phi = asin(circleRadius/sqrt(sqDistance));
      targetAngle = theta - phi;
    }
    int sodt = (targetAngle > 0) - (targetAngle < 0);
    omega = sodt;
    speed = 1;
    
    float leftSpeed = speed - omega;
    float rightSpeed = speed + omega;
